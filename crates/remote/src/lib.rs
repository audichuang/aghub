...
//! the [`ssh::Connection`] model, the [`ssh::CommandRunner`] abstraction
//! (with a real [`ssh::SystemRunner`] and a test `MockRunner`), the argv
//! builders / output parsers, and (in a later work item) the remote
//! `aghub-api` bring-up state machine. The Tauri command layer in
//! `crates/desktop/src-tauri` is a thin wrapper over this crate.
//!
