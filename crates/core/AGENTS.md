...
**`transfer.rs`**: `InstallTarget { agent, scope, project_root }`, `OperationBatchResult { results: Vec<OperationResult> }` — used for installing/copying skills to multiple agents at once. The reconcile family routes skill removal through `remove_skill_planned` (never blind `remove_dir_all` — a real directory in the shared `.agents/skills` slot is read by many agents that leave no link behind — the roster is per-descriptor, `crates/agents/tests/descriptor_regression.rs::test_{global,project}_skill_paths`). A skill discovered as a plain directory has NO master (`canonical_path` is set only for a link), so its removal plan targets the sole on-disk copy — "there is always a master to fall back on" holds for the link layout only. ONE deliberate exception, and it is what selects the layout: an unlinked Master enumerated straight out of the `.aghub` store by `skill_for_planned_removal`'s `--all-agents` fallback gets `canonical_path` SYNTHESIZED from its `source_path`, because `plan_removal` dispatches on `canonical_path.is_some()` alone and that Master must be judged by the Referrer keep rules (`plan_symlink_removal`), not the copy ones.
...
Tests that read **or** mutate `HOME`/`XDG_*` — including anything that calls `dirs::home_dir()` — must hold their binary's env lock. **One env mutex per test binary**: extend the existing one, never add a second — two mutexes serialize nobody against each other, which is how `manager::skill`'s prune tests started resolving the wrong lock file. The lib binary's is `skills::prune::test_lock::env_lock()` (`GlobalLockGuard` already holds it, and it is not reentrant — so take it once); `test_agent_paths.rs` has its own module-local `env_lock()` and api uses `test_env_lock`, both separate binaries. libtest runs one binary's tests in parallel threads of a single process, and on Unix mutating env while another thread reads it is UB; so an env swap is unsound against **any** other env-touching test **within a single test binary** — even `cargo test -p <crate> --test <name>`, not only under `--workspace`. That includes pure readers whose assertions only check path shape: a fake `$HOME` under a `Library`-containing `TMPDIR` can still flip a `!contains("Library")` assertion. Unix-gated tests: keep any helper/import they share under the same `#[cfg(unix)]`, or Windows clippy goes red. That gap is CI-only — `cargo clippy --target x86_64-pc-windows-msvc` cannot stand in for it even with the target installed, because `zstd-sys` / `aws-lc-sys` / `libsqlite3-sys` build scripts need an MSVC C toolchain — so a `cfg` slip surfaces only on push to main.
