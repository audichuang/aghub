...
impl ConfigManager {
	...
	/// from the adapter's id. The `unwrap_or` is defensive only and is
	/// unreachable in practice: the sole `impl AgentAdapter` returns a registry
	/// descriptor's `id`, which always parses. It does NOT mirror
	/// `registry::get`, which has no fallback left — that one is a total
	/// `match` generated by `agent_roster!`.
