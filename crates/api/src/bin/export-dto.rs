...
/// Emit the request limits a client has to respect. ts-rs exports SHAPES; a cap
/// is a value, so it is written here instead of hand-copied into the frontend —
/// a desktop constant that drifted below the server's would make "Update all"
/// 400 on a large Source.
fn write_limits_file(dir: &Path) -> io::Result<()> {
	let mut file = fs::File::create(dir.join("limits.ts"))?;
	writeln!(
		file,
		"// This file was generated by bin/export-dto.rs. \
	...
	writeln!(
		...
		aghub_api::dto::limits::MAX_BATCH_NAMES
	)?;
	Ok(())
}
...
fn write_index_file(dir: &Path) -> io::Result<()> {
	let mut entries = fs::read_dir(dir)?
		...
		.filter_map(|entry| {
			...
			// `limits` exports VALUES, not types — `export type { … }` would
			// erase them at compile time and the import would be undefined.
			if ext != "ts" || matches!(stem, "index" | "limits") {
				return None;
			}
			Some(stem.to_string())
		})
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	export_type::<RemovalOutcomeKind>(&cfg)?;
