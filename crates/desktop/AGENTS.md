...
- `src-tauri/` — Tauri backend; Cargo package name is **`aghub`** (`-p aghub` builds this, not the CLI), commands under `commands/`. Embeds `aghub-api` on localhost and uses `aghub-remote` for SSH; it depends on **neither** `aghub-core` nor `aghub-inference` — the app data root reaches it through the api re-export (root AGENTS.md "Where to Look"). It never opens the inference db: the provider list comes over HTTP from the embedded API. The credential wrappers it imports (`resolve_git_token_for_source` / `list_bound_sources`) DO read the OS keyring in-process.
- `src-tauri/capabilities/` — permission manifests (read `default.json`; don't restate the list here)

Page-level frontend notes: `src/AGENTS.md`.
...
After changing a Rust API DTO, run `bun run generate:dto` (the rest of the rule
is in `src/AGENTS.md`). Everything else (`dev`/`start`/`build`/`test`/`typecheck`)
is in `package.json`; **bun only**.
...
- **`Checkbox` / `Switch`: the interactive element is `.Content`, NOT the root. `.Control` MUST be nested inside `.Content`.** Since `@heroui/react` 3.2.x the root renders React Aria's `CheckboxField`/`SwitchField` — a plain context `div` — and the `<label>` + hidden `<input>` live in `CheckboxButton`/`SwitchButton`, which HeroUI exposes as `.Content`. The component's own source says so: _"`Switch.Content` — the **clickable** `SwitchButton` label wrapping the control + `Label`"_. Consequences, each verified by `renderToStaticMarkup` against the installed 3.2.5:
    ...
    - This bit EVERY compound call site in the app at once when 3.0.1 → 3.2.5 landed, and **nothing caught it**: `children` is `ReactNode`, so neither `typecheck` nor `lint` can see a missing sub-component, and the pre-push hook runs no tests. The guard that exists now is `src/lib/heroui-control-structure.test.ts` — a source scan that fails on either broken shape (it owns the call-site count, with a vacuity floor). Do not delete it.
