...
| Task                       | Location                 | Notes                                                                                                                                                                                                                                              |
| -------------------------- | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Pack / unpack `.skill`     | `src/package.rs`         | Excludes any-level `__pycache__`/`node_modules`/…; root-only `tests`/`test`/`evals`                                                                                                                                                                |
| Parse any on-disk form     | `src/parser.rs`          | Dir / `.skill` / `.zip` / `.md`                                                                                                                                                                                                                    |
| Sanitize skill name        | `src/sanitize.rs`        | `"My Skill!"` → `my-skill`                                                                                                                                                                                                                         |
| Folder content hash        | `src/hash.rs`            | npx-compatible SHA-256 (golden: `tests/hash_parity_golden.rs`). Same file also owns `compute_skill_folder_comparison_hash` (skill-update's compare basis) and `collect_skill_files` (skill-audit's input walk — the only definition of its limits) |
...
| Repo install helpers       | `src/install.rs`         | `RepoDiscoveredSkill` (the empty-lock digest the crate re-exports is `hash.rs`'s)                                                                                                                                                                  |
...
| Interprocess mutation lock | `src/lock/guard.rs`      | `std::fs::File::lock` (flock/LockFileEx), reentrant per thread, bounded; `.aghub-mutation.lock` beside the global lock / in `<proj>/.agents`                                                                                                       |
...
- **NEVER** pack root `tests/` / `test/` / `evals/` (intentionally excluded from packages)
