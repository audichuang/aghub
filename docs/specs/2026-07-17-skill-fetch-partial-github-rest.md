...
10. As a user behind a flaky network or GitHub rate limit, I want the tool to fall
    back to a git transport **when one is reachable** and succeed, so that
    correctness never depends on the REST fast-path (a total network outage still
    fails, but not because of the fast path).
...
| ------------------------------------------------------------ | ---------------------------- | ----------------------------------------- |
| CLI `source` install, API `/skills/install`, desktop install | no                           | `Skills(selected)`                        |
...
| 5   | Shallow-gix `--depth 1` fallback; correctness never depends on REST                                                     | REST-only; keep full-history fetch                                          | universal floor; also fixes today's non-shallow github fetch                                                                                                    |
