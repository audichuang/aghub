...
| GitHub Copilot CLI     | `$COPILOT_HOME/mcp-config.json`, `.mcp.json` (or `.github/mcp.json`), `mcpServers`; stdio/SSE/HTTP                                                | VS Code path/key mix-up fixed to the CLI dialect; no per-server toggle is claimed. Project MCP is first-exists-wins, never merged: `.mcp.json` else `.github/mcp.json`. [Docs](https://docs.github.com/en/copilot/how-tos/copilot-cli/customize-copilot/add-mcp-servers)      |
...
## Addendum 2026-09-06 — Oh My Pi (`omp`)

Added to the roster after this audit. Its contract was verified against
`can1357/oh-my-pi` on 2026-09-06, not on 2026-08-13.

| Agent            | Native MCP contract                                                                                                                                                                   | Aghub result                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Oh My Pi (`omp`) | `~/.omp/agent/mcp.json`, `.omp/mcp.json`, `mcpServers`; the loader accepts `transport` then `type` (`stdio`/`sse`/`http`) but the validator reads `type` alone; native `enabled` bool | `json_map` on the default `type` tag — the one spelling that both mounts and round-trips, because a `transport`-only entry defaults to stdio in the validator and is skipped at connect time rather than reported. `ToggleKey::Enabled`, the only `json_map` agent with a native toggle. An untagged remote reads as streamable HTTP, never SSE, so a hand-written `transport: "sse"` is the one config aghub cannot see. omp-owned keys (`cwd`, `oauth`, `auth`, `requestIdFormat`, `envPolicy`) survive a rewrite. The root `.mcp.json`/`mcp.json` omp also reads is deliberately NOT claimed — it belongs to Claude and Copilot. |
