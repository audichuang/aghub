...
- **Last full review**: upstream `main` @ `72f296f0` (2026-09-06) — **127 further
  upstream-only commits** since `c1801ede`. Tally: **2 ported** (`1b510d77`
  skill-audit, `bc827425` JetBrains icon) · 1 already-present · 1 N/A · **5
  deferred** (product calls, below) · ~113 skipped-or-deps (89 dependency bumps
    - ~17 `fix/feat(desktop)` on the #282 UI + 2 large parallel-solution features).
      Upstream is shipping a **1.9.0-beta** line (`v1.9.0-beta.1`, plus a beta update
      channel and beta Windows bundles); our fork keeps its own v2.x line.
      This was the first batch since `ca48d93` with substantial **Rust backend**
      work — four new crates upstream (`skill-audit`, `mcp-catalog`, `prompt`,
      `usage`), of which we took one.
- **Prior full review**: upstream `main` @ `c1801ede` (2026-07-16) — **97 further
...
- **Earlier review**: upstream `main` @ `01bf2d57` (2026-07) — **55 further
...
- **Earliest review**: upstream `main` @ `714b971` — 52 upstream-only commits since
...
| `1b510d77` (crate) | `ee241e98`  | skill-audit | offline skill security audit: YARA (cisco/clawhub/aghub rules) + injection detection + source/sink exfiltration chaining. **Fork adaptation:** the `from-path` feature and `tests/from_path.rs` were dropped — they need upstream's 664-line `skill/src/content.rs`, added by the same commit, right where v2.18 rewrote ours. Cost: `yara-x` is a git-rev dep pulling wasmtime + cranelift onto all three CI platforms                                                                                           |
| `1b510d77` (gate)  | `b75e18e5`  | core        | the audit gate itself, at OUR seams — `install_fetched_skill_and_lock` AND `resync_installed_skill`, not upstream's rewritten install path. Rationale + rejected options: `docs/adr/0002-skill-security-audit-gate.md`. Override is `--force-unsafe` on `source sync`/`apply-update`; `add` deliberately has none                                                                                                                                                                                                 |
| `bc827425`         | `5c1141f9`  | desktop     | JetBrains AI icon: `agent-icons.tsx` keys the glob by `${id}.svg` but the asset is `jetbrains_ai.svg`. The only id in the roster with that mismatch; AGENTS.md's SVG step (now step 7) had recorded it as a live bug for releases                                                                                                                                                                                                                                                                                 |
...
| Upstream                             | Note                                                                                                                  |
| ------------------------------------ | --------------------------------------------------------------------------------------------------------------------- |
| `dcc7603a` docs: api crate is Rocket | Our `AGENTS.md` already documents `crates/api` as "Rocket v0.5 under /api/v1/"; nothing to port. (`72f296f0` review.) |
...
### Deferred — product calls from the `72f296f0` review

These are real, self-contained upstream features this fork does not have. Each is
a "do we want it", not a "can we port it".

| Upstream                                         | What it is                                                                                                                                               | Porting shape                                                                                                                                                       |
| ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `d26124b3` MCP marketplace (`mcp-catalog` crate) | live search/install against the official MCP Registry (`registry.modelcontextprotocol.io`, no auth, `MCP_REGISTRY_URL` overridable); replaces a mock tab | **backend is a clean lift** (crate + `GET /mcp-market/search` + one DTO); the install modal is #282 UI and needs redoing against our MCP add dialog. Next candidate |
| `f27b8f38` rules: agent instruction files        | `rule_paths` on the descriptor + a core rules module + 3 routes; edit CLAUDE.md/AGENTS.md/GEMINI.md in-app                                               | portable — descriptor field, core module, routes; desktop page is #282 UI                                                                                           |
| `9307e73e` prompt library (`prompt` crate)       | local prompt store (`prompts.json`, atomic writes, `{{var}}` extraction) + CRUD routes                                                                   | zero coupling; the only question is whether we want the feature                                                                                                     |
| `e59a46b1` usage / ccusage (`usage` crate)       | configurable ccusage runtime + usage views (was already deferred at the `01bf2d57` review)                                                               | large, desktop-heavy; deferral stands                                                                                                                               |
| `eeb63118` provider model discovery              | fetch model lists from provider APIs                                                                                                                     | our `crates/inference` has diverged hard (+1527/−3196 vs upstream) and the desktop form is rewritten — this is a reimplementation, not a port                       |
...
| `336f6a8d` skills: manage universal + native installations (130 files) | **Parallel solution, not a port.** Upstream's own answer to divergent copies / link state — the exact ground v2.18 rewrote (`.aghub` Master, `classify`, `linker`). Adopting it means undoing v2.18.                                                                             |
| `4bea46b0` + `b938ce38` discover Codex-provided skills (79 files)      | Same reason: provider ownership + runtime visibility built on upstream's discovery, which our v2.18 discovery no longer resembles.                                                                                                                                               |
| ~17 `fix/feat(desktop)` in the `c1801ede..72f296f0` range              | Built on the agent-hub-revamp (#282) UI this fork never adopted; the target files do not exist here. Includes the settings/prompt/rule panel splits, provider drafts, agent filter labels, sidebar controls, beta update channel, what's-new wizard and store-migration renames. |
...
| `bfbd00dc` fix(release): beta Windows bundles | Our `release.yml` publishes via `softprops/action-gh-release@v3` and has no beta channel; nothing to change. |
| `5df21fde` docs(brand): lowercase aghub       | Upstream branding sweep over files this fork has rewritten.                                                  |
...
git log --no-merges --oneline 72f296f0..origin/main      # full upstream-only list + count
git log --no-merges --cherry-pick --right-only 72f296f0...origin/main    # not-yet-equivalent
