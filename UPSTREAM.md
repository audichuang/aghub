...
- **Last full review**: upstream `main` @ `72f296f0` (2026-09-06) — **127 further
  upstream-only commits** since `c1801ede`. Tally: **1 ported**
  (`bc827425` JetBrains icon) · 1 ported-then-reverted (`1b510d77` skill-audit,
  removed 2026-09-22) · 1 already-present · 1 N/A · **5
  deferred** (product calls, below) · ~113 skipped-or-deps (89 dependency bumps
    - ~17 `fix/feat(desktop)` on the #282 UI + 2 large parallel-solution features).
      ...
      `usage`), of which we took one and later removed it.
...
| `1b510d77` skill security audit (crate + gate) — **ported, then reverted** | Ported as `skill-audit` (`ee241e98`) + the gate at our two fetched seams (`b75e18e5`); both REMOVED 2026-09-22. A YARA rule set cannot tell a vendor's own `curl … \| sh` install line from an attack, and a Critical finding refused updates that were not malicious — while an aliased read of the same secret walked past it. Do not re-port: the gate's design and the rejected alternatives are kept in `docs/adr/0002-skill-security-audit-gate.md`, marked superseded. Side benefit of the revert: the `yara-x` git-rev dep (wasmtime + cranelift on all three CI platforms) is gone |
